...
public class CustomUserAccessServiceImpl implements UserDetailsService {  
    ...
    public Collection<GrantedAuthority> getAuthorities(String access) {
        ...
        authList.add(new SimpleGrantedAuthority(CommonUtil.UtilSecurity.ACCESS_ROLE_ROLE_USER));  

        if(CommonUtil.isEmptyString(access)){
        	return authList;
        }
        // 如果参数access为1.则拥有ROLE_ADMIN权限
        List<String> rights=CommonUtil.splitStringAsList(access,CommonUtil.UtilSecurity.ACCESS_STRING_SEPERATOR);
        if(rights!=null&&!rights.isEmpty()){
        	 for(String right:rights){
             	switch(right){
             		case CommonUtil.UtilSecurity.ACCESS_NUM_ROLE_ADMIN:
             			logger.debug("Grant "+CommonUtil.UtilSecurity.ACCESS_ROLE_ROLE_ADMIN+" to this user");  
                         authList.add(new SimpleGrantedAuthority(CommonUtil.UtilSecurity.ACCESS_ROLE_ROLE_ADMIN));  
             			break;
             	}
             }
        }
